/**
 * Skill Document Upload API Route
 *
 * POST /api/skills/upload
 *
 * Uploads a file to Supabase Storage and returns a public URL.
 * Used by skill input forms to provide document URLs to the skill runner.
 */
...
export async function POST(request: Request) {
	...
	if (uploadError) {
		console.error("[skills/upload] Upload error:", uploadError);
		return Response.json(
	...
	if (signedError || !signedData?.signedUrl) {
		console.error("[skills/upload] Signed URL error:", signedError);
		return Response.json(
