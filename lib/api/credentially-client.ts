...
async function credentiallyFetch<T>(
  ...
  try {
    ...
    console.log(
      `[CredentiallyAPI] Response: ${res.status} ${res.statusText}`
    );
    ...
    console.log(`[CredentiallyAPI] Success:`, JSON.stringify(data).slice(0, 200));
    return data as T;
  } catch (error) {
...
export async function createProfile(
  ...
  return credentiallyFetch<CreateProfileResponseDto>(
    `/api/${ORG_ID}/profile`,
    {
      method: "PUT",
